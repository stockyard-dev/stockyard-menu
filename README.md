...
**Self-hosted digital menu management.** Single Go binary, embedded SQLite, no external dependencies.
Part of the [Stockyard](https://stockyard.dev) suite of self-hosted developer tools.

Manage menu categories and items with prices, descriptions, dietary info, and availability toggles.

## Install

```bash
curl -fsSL https://stockyard.dev/menu/install.sh | sh
```

That downloads the latest release for your platform from
[GitHub releases](https://github.com/stockyard-dev/stockyard-menu/releases/latest)
...
Dashboard at [http://localhost:9812/ui](http://localhost:9812/ui).
HTTP API at `http://localhost:9812/api`. Data lives in `~/.stockyard/menu/`
by default — override with `-data <path>` or `DATA_DIR=<path>`.

## Personalization

Menu is one of 169 tools in the Stockyard suite. When you install it via
...
"self-hosted digital menu management" category. With it, the same binary serves a dashboard
...
|----------|---------|-------------|
| `PORT` | `9812` | HTTP listen port |
| `DATA_DIR` | `~/.stockyard/menu` | SQLite + config directory |
...
Get a Pro license at [stockyard.dev/pricing/?plan=menu-pro](https://stockyard.dev/pricing/?plan=menu-pro).
14-day free trial, cancel anytime, your data stays on your machine either way.

## Screenshot

See the live dashboard and feature tour at
[stockyard.dev/menu/](https://stockyard.dev/menu/).
